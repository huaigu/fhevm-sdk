...
function printSuccessMessage(config: GeneratorConfig): void {
  ...
  logger.title('Quick start guide:')
  logger.newLine()
  logger.info('  1️⃣  Navigate to project:')
  logger.dim(`     cd ${appName}`)
  logger.newLine()
  logger.info('  2️⃣  Terminal 1 - Start local chain:')
  logger.dim(`     ${packageManager} chain`)
  logger.newLine()
  logger.info('  3️⃣  Terminal 2 - Deploy contracts:')
  logger.dim(`     ${packageManager} deploy:localhost`)
  logger.newLine()
  logger.info('  4️⃣  Terminal 3 - Start frontend:')
  logger.dim(`     ${packageManager} dev`)
  logger.newLine()
